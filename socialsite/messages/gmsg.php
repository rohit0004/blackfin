...
if (is_dir("imgs/")) {
	...
	$groupid =filter_var(htmlspecialchars($_GET['g']),FILTER_SANITIZE_STRING);
	$getInfo = $conn->prepare("SELECT * FROM chat_group WHERE group_id=:groupid");
	$getInfo->bindParam(':groupid',$groupid,PDO::PARAM_INT);
	$getInfo->execute();
	while ($getInfo_row = $getInfo->fetch(PDO::FETCH_ASSOC)) {
		$g_id=$getInfo_row['group_id'];
		$g_name=$getInfo_row['group_name'];
		$g_image=$getInfo_row['group_image'];
		$g_member=$getInfo_row['member'];
	}
?>
<html>
<head>
...
<hr/>
 ...
 <div class="navbar-fixed">
	 ...
	 <div style="margin-left:25px;">
        <a href="<?php echo $check_path; ?>g/<?php echo $g_id ?>" class="mCol2_title"><?php echo  $g_name ?></a><br>
		<span style="float:left;margin-left:8px;font-size:11px;"><?php echo $g_member ;?> Member</span>
		</div>
...
$(document).ready(function() {
  ...
  $("#btn-poll").click(function(){
	...
	mFetchMsgs(<?php echo $groupid ?>, "timer");
 });
  $(document).on('change', '.fileUpload', function(){
   ...
   $("#sendMeg").on('click',function(){ 
	...
	type:'POST',
	url:"<?php echo $check_path; ?>includes/m_group_request.php",
	data:{'req':requ,'path':path,'gid':gid,'mid':mid,'msg':msg},
	error: function(data) {
	...
	if (data == "error") {
        ...
        $('#mSendField').val('');
        $('#mSendField').focus(); mFetchMsgs(<?php echo $groupid ?>,"timer");
         }		
	}
		});
		}
...
function mFetchMsgs(gid,type){
var requ = "fetchGroupMsgs";
var path = "<?php echo $check_path; ?>";
