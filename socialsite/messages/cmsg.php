...
if (is_dir("imgs/")) {
	...
	$channelid =filter_var(htmlspecialchars($_GET['c']),FILTER_SANITIZE_STRING);
	$getInfo = $conn->prepare("SELECT * FROM chat_channel WHERE channel_id=:channelid");
	$getInfo->bindParam(':channelid',$channelid,PDO::PARAM_INT);
	$getInfo->execute();
	while ($getInfo_row = $getInfo->fetch(PDO::FETCH_ASSOC)) {
		$c_id=$getInfo_row['channel_id'];
		$c_name=$getInfo_row['c_name'];
		$c_image=$getInfo_row['c_discription'];
		$c_member=$getInfo_row['c_member'];
	}
?>
<html>
<head>
...
<hr/>
 ...
 <div class="navbar-fixed">
	 ...
	 <div style="margin-left:25px;">
        <a href="<?php echo $check_path; ?>c/<?php echo $c_id ?>" class="mCol2_title"><?php echo  $c_name ?></a><br>
		<span style="float:left;margin-left:8px;font-size:11px;"><?php echo $c_member ;?> Member</span>
		</div>
    <span data-toggle="dropdown"  style="float:right;cursor:pointer;margin-top:0;">
    <i class=" material-icons">more_vert</i>
  </span>
    <div class="dropdown-menu">
      <a class="dropdown-item" href="#" data-count>Leave Group</a>
		  ...
		  <div class="m_SendField_box send_msg" style="display:block;">
							<div class="input-group">
								<div class="input-group-append" id="fileBtn">
									<span class="input-group-text attach_btn" style="margin:0 5px;height:40px;width: 35px;border-radius:50%;margin-top:0px;"><i class="fas fa-paperclip"></i></span>
								</div>
...
$(document).ready(function() {
  ...
  $(document).on('change', '.fileUpload', function(){
   ...
   $("#sendMeg").on('click',function(){ 
	...
	type:'POST',
	url:"<?php echo $check_path; ?>includes/m_group_request.php",
	data:{'req':requ,'path':path,'cid':cid,'mid':mid,'msg':msg},
	error: function(data) {
	...
	if (data == "error") {
	    ...
	    mFetchMsgs(<?php echo $channelid ?>,"timer");
			...
			M.toast({html: 'Please write somethimg !.', classes: 'rounded'})
		}
    mRemoveTyping(<?php echo $channelid ?>);
	});
	///////for poll////
	...
	var option = $("input[id='your_poll']")
			  ...
			  if(option != '' && question !=''){
				...
				type:'POST',
				url:"<?php echo $check_path; ?>includes/m_group_request.php",
				data:{'req':requ,'path':path,'cid':cid,'option':option,'question':question},
				error: function(data) {
				alert("There was an error. Try again please!");
				},
				success:function(data){
	...
	type:'POST',
	url:"<?php echo $check_path; ?>includes/m_requests.php",
	data:{'req':requ,'path':path,'mid':mid,'gid':gid,'msg':msg},
	error: function(data) {
	alert("There was an error. Try again please!");
	},
	success:function(data){
	if (data == "error") {
        ...
        $('#mSendField').val('');
        $('#mSendField').focus(); mFetchMsgs(<?php echo $channelid ?>,"timer");
         }		
	}
		});
		}
...
function mFetchMsgs(cid,type){
var requ = "fetchChannalMsgs";
var path = "<?php echo $check_path; ?>";
